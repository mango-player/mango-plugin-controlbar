...
var Base = /** @class */function () {
    ...
    Base.prototype.createEl = function () {
        ...
        this.option.alignRight ? this.parent.$wrapRight.appendChild(this.$dom) : this.parent.$wrapLeft.appendChild(this.$dom);
    };
...
var chimeeControl = {
    ...
    events: {
        ...
        clarityChanged: function clarityChanged(data) {
...
var ProgressBar = /** @class */function (_super) {
    ...
    // 设置关键点
    ProgressBar.prototype.initKeyPoints = function (points, duration) {
    ...
    ProgressBar.prototype.createPoints = function () {
        var _this = this;
        ...
        // 中间内容打点
        this.points = this.keyPoints.content.map(function (item) {
            var timeAndTitle = item.split('|');
            _this.createPoint(timeAndTitle[0], timeAndTitle[1]);
            return {
                time: timeAndTitle[0],
    ...
    // 创建时间轴打点数据
    ProgressBar.prototype.createPoint = function (time, title) {
        var left, $point;
        left = Math.floor(time / this.duration * 100);
        $point = (0, _mangoHelper.$)("<chimee-progressbar-keypoint \n          data-title=" + title + " data-time=" + time + " style=\"left:" + left + "%\">\n      </chimee-progressbar-keypoint>");
...
var Clarity = /** @class */function (_super) {
    ...
    Clarity.prototype.initTextList = function (clarityList, selected) {
        var _this = this;
    ...
    Clarity.prototype.click = function (e) {
        ...
        if (elem.tagName === 'LI') {
            var def = elem.getAttribute('data-def') || '';
            var name_1 = elem.getAttribute('data-name') || '';
            // 设置状态
            this.$listUl.find('li').removeClass('active');
            (0, _mangoHelper.$)(elem).addClass('active');
            this.$text.text(name_1);
            (0, _mangoHelper.removeClassName)(this.$dom, 'on');
            // 发送请求事件
            this.parent.emit('switchClarity', { def: def });
        }
    };
    Clarity.prototype.clarityChanged = function (data) {
        if (data.error) {
            console.log('切换清晰度失败');
        }
    };
